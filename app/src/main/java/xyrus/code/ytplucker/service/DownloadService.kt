...
class DownloadService : Service() {
        ...
        val job = serviceScope.launch {
            ...
            try {
                ...
                suspend fun runOnce() = repository.downloadManager.download(
                    ...
                    cookiesPath = cookiesPath, onProgress = onProgress,
                )
                try {
                    runOnce()
                } catch (e: Exception) {
                ...
                cookiesPath?.let { File(it).delete() }
    ...
    /**
     * If the URL matches a known platform and saved cookies exist, write a Netscape-format
     * cookie file to the cache dir and return its path. Returns null when no cookies are
     * available, in which case yt-dlp runs unauthenticated.
     */
    private fun resolveCookies(url: String, jobId: String): String? {
        val platform = platformForVideoUrl(url) ?: return null
        val app = applicationContext as YtPluckerApp
        val cookieStr = app.preferences.getCookies(platform.cookieKey) ?: return null
        val file = File(cacheDir, "cookies/$jobId.txt")
        file.parentFile?.mkdirs()
        file.writeText(toNetscapeFormat(cookieStr, platform.cookieDomain))
        return file.absolutePath
    }

    /**
     * Convert a raw cookie string from CookieManager.getCookie() (e.g.
     * "name1=value1; name2=value2") into Netscape-format cookie file content that
     * yt-dlp's --cookies option can read.
     */
    private fun toNetscapeFormat(cookieString: String, domain: String): String {
        val host = domain.removePrefix("https://").removePrefix("http://")
        val bareDomain = host.removePrefix("www.").removePrefix("m.")
        val netscapeDomain = ".$bareDomain"
        val expiry = (System.currentTimeMillis() / 1000) + 365L * 24 * 60 * 60
        val sb = StringBuilder()
        sb.appendLine("# Netscape HTTP Cookie File")
        sb.appendLine("# Generated by YT-Plucker")
        cookieString.split(";").forEach { pair ->
            ...
            if (eqIdx > 0) {
                ...
                sb.appendLine("$netscapeDomain\tTRUE\t/\tTRUE\t$expiry\t$name\t$value")
            }
        }
        return sb.toString()
    }
